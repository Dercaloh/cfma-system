...
<x-app-layout :header="__('Detalle del Préstamo')">
    ...
    <x-glass-card title="Resumen del Préstamo">
        <ul class="space-y-2 text-base leading-6 text-gray-800">
            ...
            @if($loan->approved_at)
                <li><strong>Aprobado:</strong> {{ $loan->approved_at->format('d/m/Y H:i') }}</li>
            @endif
            @if($loan->delivered_at)
                <li><strong>Entregado:</strong> {{ $loan->delivered_at->format('d/m/Y H:i') }}</li>
            @endif
            @if($loan->returned_at)
                <li><strong>Devuelto:</strong> {{ $loan->returned_at->format('d/m/Y H:i') }}</li>
            @endif
