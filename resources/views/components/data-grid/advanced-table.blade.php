...
<div class="w-full space-y-4" x-data="advancedTable()" x-init="init()">
    ...
    <div class="p-4 space-y-4 card-glass">
        ...
        @if($filterable)
        <div x-show="showFilters" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 max-h-0" x-transition:enter-end="opacity-100 max-h-96" x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100 max-h-96" x-transition:leave-end="opacity-0 max-h-0" class="overflow-hidden">
                        ...
                        <tr class="transition-colors hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">
                                ...
                                <x-ui.status-badge
                                    ...
                                    :type="$column['badge_type'] ?? 'default'" />
                                ...
                                <span x-text="formatCurrency(row.{{ $column['key'] }})"></span>
                                @elseif(isset($column['type']) && $column['type'] === 'image')
                                <img :src="row.{{ $column['key'] }}"
                                     class="object-cover w-10 h-10 rounded-full"
                                     :alt="'Imagen de ' + row.name || 'Imagen'"
                                     loading="lazy">
                                @else
