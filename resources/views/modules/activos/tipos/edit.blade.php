<x-app-layout>
    <x-slot name="header">
        <x-layout.context-header
            ...
            subtitle="Modifica la información de: {{ $assetType->name }}"
        >
    ...
    <x-cards.card-glass class="mt-6">
        ...
        <form action="{{ route('admin.tipos_activos.update', $assetType) }}" method="POST" class="p-6 space-y-6">
            ...
            {{-- Botones de acción --}}
            <x-buttons.form-action-buttons
                cancelHref="{{ route('admin.tipos_activos.index') }}"
                cancelText="Cancelar"
